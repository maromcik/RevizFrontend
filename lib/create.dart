...
class _CreateDeviceState extends State<CreateDevice> {
  ...
  _searchDevice() async {
    try {
      await Utils.retrieveDevice(widget.client, controllerQrText.text)
  ...
  _createDevice() async {
    ...
    bool exist = false;
    await Utils.existDevice(widget.client, controllerQrText.text).then((value) {
      exist = value;
    });
    if (!exist) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Create or modify device")),
      body: Column(
        children: [
          TextFormField(
              ...
              validator: (value) {
                if (value == null || value.isEmpty) {
                  return "Device QR code cannot be blank";
                }
                return null;
              }),
          ...
          Row(
            ...
            children: [
              ...
              ElevatedButton(
                  onPressed: () async {
                    if (controllerQrText.text.isEmpty) {
                      print("Device QR code cannot be blank");
                      return;
                    }
                    _createDevice();
                  },
